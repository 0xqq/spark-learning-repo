...
class ReliableKafkaReceiver[
    ...
    */
    ...
    // 下面定义的这个类, 是用来处理由 BlockGenerator 生成的 block 对象的
    // 而这里可以注意到 BlockGeneratorListener 这个接口/特质/trait 是定义在 BlockGenerator 这个类中的
    // 而, 是的没错, 这个 BlockGenerator 类也是很值得读一下的, 乍一看我还以为这个 Listener 是继承了 SparkListener 派系的一个监听器
    // 接收 SparkListenerBus 丢给各个子 Listener 的各种 Event 进行处理, 但是这个 BlockGeneratorListner 并不是属于  SparkListener 的
    // BlockGeneratorListener 中定义的 onAddData/onGenerateBlock/onPushBlock/onError 这几个方法分别定义了, 在 BlockGenerator 中
    // 响应不同的 event 而调用的不同的回调方法, 而不同的回调方法由继承 BlockGeneratorListener 类的子类来给出实现逻辑
    
    private final class GeneratedBlockHandler extends BlockGeneratorListener {
        
        def onAddData(data:Any, metadata:Any):Unit = {
        	// Update the offset of the data that was added to the generator 
        	// 这个 onAddData 函数只有当数据被成功写入 block 的时候才会触发执行, 
        	// 所以在这个方法中加入更新 offset 的逻辑, 或者是在这个时间点更新 offset 是最合适不过的了
        	// 而更新 offset 的函数便是 updateOffset 
        	if (metadata != null) {
        		// 首先先将传入的 metadata 的类型从 Any 转换为 (TopicAndPartition,Long) 元组类型
        ...
        def onGenerateBlock(blockId:StreamBlockId):Unit = {
        	...
        	// 这个方法在 BlockGenerator 构建一个新的 Block 对象的时候回调用, 
        	// 而传入的这个 blockId 是 BlockGenerator 生成 Block 对象的时, 为这个 Block 所生成的唯一标识符 ID 数值
        	// 我们在整个 ReliableKafkaReceiver 生命周期内维护了一个 hashmap 
        	// 每次在构建 block 的周期内会构建一批 topic.partition - offset 的 key value 对
        	// 当数据累计一定程度便会构建一个 block, 这个时候将 topic.parititon -offset key-value 对转换为 immutable 类型的 map 
        	// 然后通过 rememberBlockOffsets 函数将 blockId - immutable hash map 构建关联, 一一映射
        	rememberBlockOffsets(blockId)
        }

        def onPushBlock(blockId:StreamBlockId, arrayBuffer:mutable.ArrayBuffer[_]):Unit = {
        	// Store block and commit the blocks offset 
        	// 如果说前面只是生成或是更新 blockId 和这个 blockId 对应的上游数据流的 offset 数值这些在本地的操作话, 
        	// 那么这个 storeBlockAndCommitOffset 方法则是将 blockId 后跟着的数据转换为
        	// 数据流, 然后将 blockId 和与该 blockId 关联的各个 topic.partition 下的 offset 信息提交到远程记录 offset 数值的 zookeeper 上
